...
### 15. Логика подсветки при смене контекста (Highlight Persistence)
* **Проблема:** Переменная `_highlightTaskId` (мигание) должна корректно отрабатывать при перемещении задачи между табами (Active -> Triumph).
* **Риск:** Если не устанавливать подсветку явно при действиях `complete` или `delete`, пользователь теряет визуальный фидбек о том, куда улетела задача при переходе в соответствующий архив.

### 16. Ограничения Scroll Physics и Tap Gestures
* **Проблема:** Попытка блокировать тапы во время инерционного скролла через `NotificationListener` и флаг `isScrolling` привела к лагам и плохой отзывчивости интерфейса. Пользователь ощущает, что приложение "тупит".
* **Решение:** Отказаться от блокировки тапов при скролле. Оставить стандартное поведение Flutter (меню может открыться при тапе по движущемуся элементу), так как отзывчивость важнее.

### 17. Неинтрузивные уведомления (Layout Shift)
* **Концепция:** Важные, но не блокирующие уведомления (как "Дубликат") не должны перекрывать контент (Overlay).
* **Реализация:** Использовать `Column` и блок внизу экрана, физически сдвигая список задач вверх.

### 18. Стиль приложения и приемственность стилей
